...
class RAGFlowAnimation(Scene):
    def construct(self):
        ...
        gateway = flow.add_node("API Gateway", subtitle="FastAPI")
        vdb = flow.add_node("Vector DB", subtitle="Qdrant")
        llm = flow.add_node("LLM", subtitle="Claude 3.5")

        flow.connect(client, gateway)
        flow.connect(gateway, vdb)
        flow.connect(gateway, llm)

        flow.auto_layout(mode="horizontal", spacing=2.2)

        # 2. Add narrative actions
        flow.timeline.reveal_sequence(delay_per_item=0.3)
        flow.timeline.send_packet(client, gateway, duration=0.8)
        flow.timeline.send_packet(gateway, vdb, duration=0.8)
        flow.timeline.highlight_node(vdb, status="active", duration=0.6)
        flow.timeline.send_packet(gateway, llm, duration=0.8)
        flow.timeline.highlight_node(llm, status="success", duration=0.8)
